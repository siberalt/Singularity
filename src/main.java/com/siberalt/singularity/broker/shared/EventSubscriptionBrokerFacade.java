...
public class EventSubscriptionBrokerFacade extends BrokerFacade{
    ...
    public <T extends Event> Subscription subscribe(SubscriptionSpec<T> subscriptionSpec, EventHandler<T> handler) {
        return broker.getSubscriptionManager().subscribe(subscriptionSpec, handler);
    }
}
